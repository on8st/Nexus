...
| ~~`usbtopo` label rewriting — `label_by_rig`, `label_serial_ports`, `devices_sharing_usb_device`~~ **REMOVED 2026-09-06** | Upstream merged #109 as structured facts only — its header states it "does not rename anything… the pickers keep showing exactly the strings they showed before". The fork's renaming was dropped and upstream's `usbtopo` adopted wholesale, on the operator's decision: a permanent divergence in a file upstream actively maintains costs more than the label suffix was worth. `audio_devices_for_port` and the "— this radio" suffix went with it. | closed — converged on upstream |
