...
# FTDI's dylibs carry install names the loader cannot resolve, and it costs a puzzling launch-time
# failure rather than a build error:
#   libftd2xx  -> "@rpath/libftd2xx…"   and cargo embeds no LC_RPATH  ("no LC_RPATH's found")
#   libft4222  -> "build-arm64/libft4222…", a RELATIVE path, resolved against the working directory
# Rewrite them in OUR binary rather than theirs: `install_name_tool -id` on their dylib would be
# modifying FTDI's file, which their licence forbids (clause 3.1.2), and this achieves the same thing
# on a file that is entirely ours.
if [ "$YAESU_WF" = 1 ]; then
  app="$REPO/src-tauri/target/release/bundle/macos/Nexus.app"
  bin="$app/Contents/MacOS/Nexus"
  [ -f "$bin" ] || die "--yaesu-wf: no binary at $bin to repoint"
  changed=0
  while read -r ref; do
    case "$ref" in
      *libft4222*|*libftd2xx*)
        base="$(basename "$ref")"
        [ -f "$FT4222_LIB/$base" ] || die "--yaesu-wf: the binary wants $base but it is not in $FT4222_LIB"
        install_name_tool -change "$ref" "$FT4222_LIB/$base" "$bin" || die "install_name_tool failed for $ref"
        changed=$((changed+1))
        ;;
    esac
  done < <(otool -L "$bin" | awk 'NR>1 {print $1}')
  [ "$changed" -gt 0 ] || die "--yaesu-wf: the binary references no FTDI library — did the feature actually build?"
  # install_name_tool invalidates the signature; re-apply the ad-hoc one the build produced.
  codesign -f -s - "$bin" >/dev/null 2>&1 || warn "could not re-sign after repointing (unsigned build)"
  ok "FTDI libraries repointed to absolute paths in the app ($changed references)"
  # Prove it WITHOUT launching the app. Launching is not available as a control here: there is no
  # --version flag, so the app boots the whole GUI, dials the clusters and REWRITES settings.json
  # before exiting 0 — a build step must not do that, and the exit code would not discriminate
  # anyway. Read the load commands instead, including the ones inside FTDI's own dylibs, which is
  # where the last trap lives.
  missing=0
  for ref in $(otool -L "$bin" | awk 'NR>1 {print $1}' | grep -E 'libft4222|libftd2xx'); do
    [ -f "$ref" ] || { warn "the app records $ref, which does not exist"; missing=1; }
  done
  [ "$missing" = 0 ] || die "--yaesu-wf: the app records FTDI paths that are not on disk"
  ok "every FTDI path the app records exists on disk"

  # THE ONE WE CANNOT REWRITE. libft4222 declares its own dependency on libftd2xx as a BARE LEAF
  # NAME — "libftd2xx.dylib", no @rpath, no directory — so dyld can only reach it through its
  # search paths, and no rewrite of OUR binary helps a dependency-of-a-dependency. Left unsaid,
  # this surfaces on the operator's machine as a crash report whose only clue is the termination
  # indicator "Library missing" (it cost exactly that on 2026-08-19). Say it here instead.
  bare=""
  for lib in "$FT4222_LIB"/libft4222*.dylib; do
    [ -f "$lib" ] || continue
    for dep in $(otool -L "$lib" | awk 'NR>1 {print $1}'); do
      case "$dep" in
        /*|@*) ;;                       # absolute or @rpath/@loader_path — dyld can resolve these
        *libftd2xx*) bare="$dep" ;;
      esac
    done
  done
  if [ -n "$bare" ]; then
    if [ -f "/usr/local/lib/$bare" ]; then
      ok "FTDI's bare dependency '$bare' resolves via /usr/local/lib"
    else
      warn "libft4222 depends on '$bare' with NO path, so dyld has to search. Either:"
      warn "  - launch through the wrapper written below (it sets DYLD_LIBRARY_PATH), or"
      warn "  - sudo ln -s '$FT4222_LIB/libftd2xx.1.4.35.dylib' '/usr/local/lib/$bare'"
      warn "    which is FTDI's own documented install location and also makes a double-click work."
    fi
  fi

  # A wrapper, because DYLD_LIBRARY_PATH cannot be baked into a bundle and Finder passes no env.
  launcher="$app/../run-nexus-wf.sh"
  cat > "$launcher" <<'WRAP'
#!/bin/sh
# Launch this RF-scope build with FTDI's libraries reachable. Generated by scripts/build-macos.sh.
# Needed only because libft4222 names its own libftd2xx dependency with no path; see that script.
FT4222_LIB="${FT4222_LIB:-$HOME/.local/lib/ft4222}"
BIN="$(cd "$(dirname "$0")" && pwd)/Nexus.app/Contents/MacOS/Nexus"
[ -x "$BIN" ] || { echo "no build at $BIN" >&2; exit 1; }
DYLD_LIBRARY_PATH="$FT4222_LIB${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}" exec "$BIN" "$@"
WRAP
  chmod +x "$launcher"
  ok "wrapper: $(cd "$(dirname "$launcher")" && pwd)/run-nexus-wf.sh"
fi
