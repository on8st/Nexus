...
describe('waterfall scroll direction', () => {
  it('defaults to today’s direction with nothing stored, and writes nothing until asked', () => {
    const btn = mount()
  ...
  it('a stale or foreign stored value is the default too — only `down` flips it', () => {
    for (const stored of ['', 'up', '1', 'DOWN', 'true']) {
      localStorage.setItem(FLOW_KEY, stored)
      const btn = mount()
      expect(btn.textContent, `stored=${JSON.stringify(stored)}`).toBe('Scrolls up')
      expect(btn.getAttribute('aria-pressed')).toBe('false')
      cleanup()
    }
  })
  ...
  it('reopens in the direction the operator left it', () => {
    localStorage.setItem(FLOW_KEY, 'down')
    const btn = mount()
    expect(btn.textContent).toBe('Scrolls down')
    expect(btn.getAttribute('aria-pressed')).toBe('true')
  })
  ...
  it('the tooltip says which END is newest and which way history travels, in both states', () => {
    ...
    expect(up).toMatch(/newest at the top/i) // what clicking does
    fireEvent.click(btn)
    ...
    expect(down).toMatch(/newest at the bottom/i)
  })
